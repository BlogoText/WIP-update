        ...
        foreach ($code_before as $i => $code) {
            ...
            if ($pos !== false) {
                 $texte = substr_replace(
                            $texte,
                            '<pre'.((isset($code_after[$i][1])) ? $code_after[$i][1] : '').'><code>'
                                .htmlspecialchars(htmlspecialchars_decode($code_before[$i][3])).'</code></pre>',
                            $pos,
                            mb_strlen($code_after[$i][0])
                        );
            }
        }
    }
...
function parse_texte_paragraphs($texte)
{
    ...
    $block_elements = 'address|article|aside|audio|blockquote|canvas|dd|li|div|[oud]l|fieldset|'
                     .'fig(caption|ure)|footer|form|h[1-6]|header|hgroup|hr|main|nav|noscript|'
                     .'output|p|pre|prebtcode|section|table|thead|tbody|tfoot|tr|td|video';
    ...
    while ($finished === false) {
        ...
        if (preg_match('#^<('.$block_elements.') ?.*?>(.*?)</(\1)>#s', $texte_formate, $matches)) {
            ...
            $finished = (mb_strlen($texte_retire) === 0) ? true : false;
        } else {
            ...
            $texte_retire = trim(substr($texte_formate, 0, -mb_strlen($texte_restant)));
            // IF greped text is empty: no text or no further block element (or new line)
            if (mb_strlen($texte_retire) === 0) {
                // remaining text is NOT empty : keep it in a <p></p>
                if (mb_strlen($texte_restant) !== 0) {
...
function tags_aside($mode)
{
    ...
    $all_tags = tags_list_all($where, 1);

    // attach non-diacritic versions of tag, so that "é" does not pass after "z" and re-indexes
    foreach ($all_tags as $tag => $ct) {
        $all_tags[$tag] = array(diacritique(trim($tag)), $ct);
    }
    // sort tags according non-diacritics versions of tags
    $all_tags = array_reverse(sort_by_subkey($all_tags, 0));
    $uliste = '<ul>';
    ...
    foreach ($all_tags as $tag => $ct) {
        if ($tag != '' and $ct[1] > 1) {
            $uliste .= '<li>';
            $uliste .=  '<a href="?tag='.urlencode(trim($tag)).$ampmode.'" rel="tag">'.ucfirst($tag).' ('.$ct[1].')</a>';
            $uliste .=  '<a href="rss.php?tag='.urlencode($tag).$ampmode.'" rel="alternate"></a>';
            $uliste .= '</li>';
        }
    }
    $uliste .= '</ul>';
...
function lien_pagination()
{
    ...
    $page_current = (isset($_GET['p']) and is_numeric($_GET['p'])) ? (int)$_GET['p'] : 0;
    ...
    $ct = (int)db_items_list_count($db_req, $db_params);

    $link_previous = '';
    $link_next = '';
    // -1 because ?p=0 is the first
    $total_page = (int)ceil($ct / $nb_par_page) - 1;

    // page sup ?
    if ($page_current < 0) {
        $link_next = '<a href="?'.$qstring.'p=0" rel="next">'.$GLOBALS['lang']['label_next'].'</a>';
    } else if ($page_current < $total_page) {
        $link_next = '<a href="?'.$qstring.'p='.($page_current+1).'" rel="next">'.$GLOBALS['lang']['label_next'].'</a>';
    }

    // page inf ?
    if ($page_current > $total_page) {
        $link_previous = '<a href="?'.$qstring.'p='.$total_page.'" rel="prev">'.$GLOBALS['lang']['label_previous'].'</a>';
    } else if ($page_current <= $total_page && $page_current > 0) {
        $link_previous = '<a href="?'.$qstring.'p='.($page_current-1).'" rel="prev">'.$GLOBALS['lang']['label_previous'].'</a>';
    }

    return '<p class="pagination">'.$link_previous.$link_next.'</p>';
}
...
function liste_tags($billet, $html_link)
{
    ...
    if (!empty($billet['bt_tags'])) {
        ...
        foreach ($tag_list as $tag) {
            $tag = trim($tag['t']);
            if ($html_link == 1) {
                $list .= '<a href="?tag='.urlencode($tag).$mode.'" rel="tag">'.$tag.'</a>';
            } else {
                $list .= $tag.' ';
            }
        }
