...
require_once BT_ROOT.'admin/inc/quickly.php';
...
function install_form_1_echo($errors = '')
{
    ...
    if (!is_writable('../')) {
        $confErrors[] = '<li>BlogoText has no write rights (recommended : chmod 755 for the folders and 644 for the files).</li>';
    }
    if ($confErrors) {
...
function install_form_2_echo($errors = '')
{
    ...
    echo '<p>';
    echo '<label for="racine">'.$GLOBALS['lang']['settings_site_url_label'].' </label><input type="url" name="racine" id="racine" size="30" value="'.$link.'" class="text" required />';
    echo '</p>';
    echo hidden_input('comments_defaut_status', 1);
...
function install_form_2_proceed()
{
    ...
    folder_create(DIR_CONFIG, true);
    folder_create(DIR_IMAGES, false);
    folder_create(DIR_DOCUMENTS, false);
    folder_create(DIR_VHOST_DATABASES, true);
...
function install_form_3_proceed()
{
    ...
    $totalPosts = db_items_list_count('SELECT count(ID) AS counter FROM articles', array());
    ...
    if ($GLOBALS['db_handle']) {
        ...
        if (!blog_db($post1, 'enregistrer-nouveau')) {
            die('ERROR SQL posting first article.');
        }
        blog_db($post2, 'enregistrer-nouveau');
