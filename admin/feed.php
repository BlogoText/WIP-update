...
function display_form_feed_conf($errors = '')
{
    ...
    $out = '<form id="form-rss-config" class="main-white" method="post" action="feed.php?config">';
    $out .= '<ul>';
    foreach ($GLOBALS['feeds_list'] as $flux) {
        $out .= '<li>';
            $out .= '<span'.( ($flux['iserror'] > 2) ? ' class="feed-error" title="('.$flux['iserror'].' last requests were errors.)" ' : ''  ).'>';
                $out .= '<label for="i_'.$flux['checksum'].'">'.$GLOBALS['lang']['rss_label_title_feed'].'</label>';
                $out .= '<input id="i_'.$flux['checksum'].'" name="i_'.$flux['checksum'].'" type="text" class="text" value="'.htmlspecialchars($flux['title']).'">';
            $out .= '</span>';
            $out .= '<span>';
                $out .= '<label for="j_'.$flux['checksum'].'">'.$GLOBALS['lang']['rss_label_url_feed'].'</label>';
            ...
            $out .= '</span>';
            $out .= '<span>';
                $out .= '<button type="button" class="btn btn-cancel" onclick="unMarkAsRemove(this)">'.$GLOBALS['lang']['cancel'].'</button>';
                $out .= '<button type="button" class="btn btn-delete" onclick="markAsRemove(this)">'.$GLOBALS['lang']['supprimer'].'</button>';
            $out .= '</span>';
        $out .= '</li>';
    }
    $out .= '</ul>';
...
function traitment_form_feed_conf()
{
    ...
    foreach ($GLOBALS['feeds_list'] as $idx => $feed) {
        ...
        if ($title) {
            ...
            // Title has change
            $GLOBALS['feeds_list'][$idx]['title'] = $title;

            // Folder has changed: update & change folder where it must be changed
            if ($folder != $GLOBALS['feeds_list'][$idx]['folder']) {
                $GLOBALS['feeds_list'][$idx]['folder'] = $folder;
                $req = $GLOBALS['db_handle']->prepare('UPDATE rss SET bt_folder = ? WHERE bt_feed = ?');
                $req->execute(array($folder, $feed['link']));
            }

            // URL has change
            if ($link != $GLOBALS['feeds_list'][$idx]['link']) {
                $newUrl = $GLOBALS['feeds_list'][$idx];
                $newUrl['link'] = $link;
                unset($GLOBALS['feeds_list'][$idx]);
                $GLOBALS['feeds_list'][$newUrl['link']] = $newUrl;
...
function feed_list_html($selected = '')
{
    ...
    $folders = array();
    foreach ($GLOBALS['feeds_list'] as $feed) {
...
function send_rss_json($feeds)
{
    ...
    foreach ($feeds as $feed) {
        $to_json['list'][] = array(
            ...
            'time' => time_formate(date('YmdHis', $feed['bt_date'])),
            'title' => $feed['bt_title'],
            'link' => $feed['bt_link'],
            'feed' => $feed['bt_feed'],
            'sitename' => (!is_utf8($GLOBALS['feeds_list'][$feed['bt_feed']]['title'])) ? utf8_encode($GLOBALS['feeds_list'][$feed['bt_feed']]['title']) : $GLOBALS['feeds_list'][$feed['bt_feed']]['title'],
            'folder' => $GLOBALS['feeds_list'][$feed['bt_feed']]['folder'],
    ...
    return '<script>var rss_entries = '. json_encode($to_json, JSON_UNESCAPED_UNICODE) .';</script>';
}
...
echo tpl_get_html_head($GLOBALS['lang']['my_subs'], false);

// echo '<div id="header">';
    // echo '<div id="top">';
        // tpl_show_msg();
        // echo tpl_get_search_form();
        // echo tpl_show_topnav($GLOBALS['lang']['my_subs']);
    // echo '</div>';
...
if ($config !== null) {
    ...
    $out .= '<ul id="feed-list" class="main-white">';
    ...
    $out .= '<div id="post-list-wrapper">';
        $out .= '<div id="post-list-title">';
            $out .= '<ul class="rss-menu-buttons">';
                // mark as read
    if ($site) {
        $out .= '<li><button type="button" onclick="sendMarkReadRequest(\'site\', \''.$site.'\', false);" id="markasread" title="'.$GLOBALS['lang']['rss_label_markasread'].'"></button></li>';
    } elseif ($fold) {
        $out .= '<li><button type="button" onclick="sendMarkReadRequest(\'folder\', \''.$fold.'\', true);" id="markasread" title="'.$GLOBALS['lang']['rss_label_markasread'].'"></button></li>';
    } else {
        $out .= '<li><button type="button" onclick="sendMarkReadRequest(\'all\');" id="markasread" title="'.$GLOBALS['lang']['rss_label_markasread'].'"></button></li>';
    }
                // open all
                ...
                $out .= $btn_next_page;
            $out .= '</ul>';
            $out .= '<p><span id="post-counter"></span> '.$GLOBALS['lang']['label_items'].'</p>';
        $out .= '</div>';

    // Here comes (in JS) the <ul id="post-list"></ul>
    if (empty($GLOBALS['feeds_list'])) {
