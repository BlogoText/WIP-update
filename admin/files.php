...
function display_form_file($errors, $files)
{
    ...
    $form = '<form id="form-image" class="form main-white" enctype="multipart/form-data" method="post" action="'.basename($_SERVER['SCRIPT_NAME']).'" onsubmit="submitdnd(event);">';

    if (!$files) {
        // No file, display the form to add a new one
        $form .= '<fieldset >';
        $form .= '<div style="display: inline-block;width:49%">';
            $form .= '<div id="form-dragndrop">';
                $form .= '<div id="dragndrop-area" ondragover="event.preventDefault();" ondrop="handleDrop(event);" >';
                    $form .= '<div id="dragndrop-title">';
                        ...
                        $form .= '<div class="upload-info">('.$GLOBALS['lang']['label_up_to'].$maxFileSize.$GLOBALS['lang']['label_per_file'].')</div>';
                    $form .= '</div>';
                    $form .= '<p>'.$GLOBALS['lang']['ou'].'</p>';
                    $form .= '<div id="file-input-wrapper">';
                        $form .= '<input name="fichier" id="fichier" class="text" type="file" />';
                        $form .= '<label for="fichier"></label>';
                    $form .= '</div>';
        ...
        $form .= '<div style="display: inline-block;width:49%">';
            ...
            $form .= '<div id="img-others-infos" class="form-inline block-white">';
                $form .= '
                    <div class="input">
                        <input type="text" id="nom_entree" name="nom_entree" placeholder="'.$GLOBALS['lang']['placeholder_filename'].'" value="" class="text" />
                        <label for="nom_entree" title="'.$GLOBALS['lang']['label_dp_nom'].'"></label>
                    </div>
                    <div class="input">
                        <textarea class="text" id="description" name="description" rows="5" placeholder="'.$GLOBALS['lang']['placeholder_description'].'" ></textarea>
                        <label for="description" title="'.$GLOBALS['lang']['label_dp_description'].'"></label>
                    </div>
                    <div class="input">
                        <input type="text" id="dossier" name="dossier" placeholder="'.$GLOBALS['lang']['placeholder_folder'].'" value="" class="text" />
                        <label for="dossier" id="'.$GLOBALS['lang']['label_dp_folder'].'"></label>
                    </div>
                    <div class="input">
                        <input type="checkbox" id="statut" name="statut" class="checkbox" />
                        <label for="statut">'.$GLOBALS['lang']['label_file_private'].'</label>
                    </div>';
        ...
        $form .= '</div>';
            ...
            $form .= hidden_input('token', token_set(), 'id');
                $form .= '<button class="btn btn-submit" type="submit" name="upload">'.$GLOBALS['lang']['img_upload'].'</button>';
            $form .= '</div>';
        ...
        if ($myFile['bt_type'] == 'image') {
            ...
            $form .= '<li><b>'.$GLOBALS['lang']['label_dp_date'].'</b>'.date_formate($myFile['bt_id']).', '.time_formate($myFile['bt_id']).'</li>';
        ...
        $form .= '<p><label for="dossier">'.$GLOBALS['lang']['label_dp_folder'].'</label><input type="text" name="dossier" placeholder="'.$GLOBALS['lang']['placeholder_folder'].'" value="'.((!empty($myFile['bt_dossier'])) ? $myFile['bt_dossier'] : '').'" size="60" class="text" /></p>';
        $checked = ($myFile['bt_statut'] == 0) ? 'checked ' : '';
        $form .= '<p><input type="checkbox" id="statut" name="statut" '.$checked.' class="checkbox" /><label for="statut">'.$GLOBALS['lang']['label_file_private'].'</label></p>';
        ...
        $form .= '<button class="submit button-cancel" type="button" onclick="redirection(\'files.php\');">'.$GLOBALS['lang']['cancel'].'</button>';
...
function display_files_list($arr)
{
    ...
    $lstype = array();
    $arr = sort_by_subkey($arr, 'bt_type');
    foreach ($arr as $file) {
        $lstype[$file['bt_type']] = (isset($lstype[$file['bt_type']])) ? $lstype[$file['bt_type']]+1 : 1 ;
    }

    $out = '<div class="main-white" id="files-section">';
    // Buttons
    if ($lstype) {
    ...
    $out .= '<thead>';
        $out .= '<tr><th></th><th>'.$GLOBALS['lang']['label_dp_nom'].'</th><th>'.$GLOBALS['lang']['label_dp_filesize'].'</th><th>'.$GLOBALS['lang']['label_dp_date'].'</th><th></th><th></th></tr>';
    $out .= '</thead>';
    $out .= '<tbody>';
    ...
    foreach ($arr as $file) {
        ...
        $out .= '<td><a href="files.php?file_id='.$file['bt_id'].'&amp;edit">'.$file['bt_filename'].'</a></td>';
...
function traitment_form_file($file)
{
    ...
    if ($vars['upload']) {
        ...
        // via $_POST d’une url
        if ($vars['fichier']) {
            $newFile = file_handler_add($file, 'download', $vars['fichier']);
        }
...
function display_pictures_list($images)
{
    ...
    foreach ($images as $idx => $im) {
        ...
        $thumb_name = file_thumbnail_path($img_path_src);
    ...
    // Pictures
    $out .= '<div class="main-white" id="image-section">';
    // Buttons
    if ($arrUniq) {
...
if ($vars['filtre']) {
    ...
    foreach ($GLOBALS['files_list'] as $fich) {
...
echo tpl_get_html_head($GLOBALS['lang']['title_files'], true);
...
if ($vars['ajout']) {
    ...
    // Subnav
    echo '<div id="subnav">';
    echo tpl_filter_form('fichiers', htmlspecialchars($vars['filtre']));
    echo '</div>';
