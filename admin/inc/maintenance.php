...
function rebuilt_file_db()
{
    ...
    foreach ($idir as $file) {
        ...
        if (!in_array($file, $filesDtb)) {
            ...
            $GLOBALS['files_list'][] = $newImg;
        }
        file_thumbnail_create($filepath);
    }
...
function create_html_favs($numberOfLinks)
{
    ...
    $sql = '
        SELECT *
         ...
         ORDER BY bt_id DESC '.
         $limit;
    $list = db_items_list($sql, array(), 'links');
    ...
    foreach ($list as $link) {
        ...
        $timestamp = mktime($dec['hour'], $dec['minutes'], $dec['seconds'], $dec['month'], $dec['day'], $dec['year']);  // HISMDY : wtf!
    ...
    return (file_put_contents(DIR_VHOST_BACKUP.$path, $html, LOCK_EX) === false) ? false : URL_VHOST_BACKUP.$path;
}
    ...
    foreach ($arrDiff as $com) {
        $query = '
               INSERT INTO commentaires 
                         (
                            bt_type, bt_id, bt_article_id, 
                            bt_content, bt_wiki_content, 
                            bt_author, bt_link, bt_webpage, 
                            bt_email, bt_subscribe, bt_statut
                          )
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        $req = $GLOBALS['db_handle']->prepare($query);
        $req->execute(
...
function recompte_comments()
{
    if (DBMS == 'sqlite') {
        $query = '
            UPDATE articles
               SET bt_nb_comments = COALESCE(
                        (
                             SELECT count(a.bt_id)
                               FROM articles a
        ...
        $query = '
            UPDATE articles
               SET bt_nb_comments = COALESCE(
                        (
                             SELECT count(articles.bt_id)
                               FROM commentaires
                              WHERE commentaires.bt_article_id = articles.bt_id
                        ),
                        0
                    )';
    }
...
/**
 * add folder to a zip instance
 * this function is recursive
 *
 * @params $zip obj, the zip instance
 * @params $path string, path to the folder
 */
function addFolder2zip($zip, $path)
{
...
{
    $zipfile = 'archive_site-'.date('Ymd').'-'.substr(md5(mt_rand(10, 99)), 3, 5).'.zip';
    $zip = new ZipArchive;
    if ($zip->open(DIR_VHOST_BACKUP.$zipfile, ZipArchive::CREATE) === true) {
        foreach ($folders as $folder) {
            addFolder2zip($zip, $folder);
        }
        $zip->close();
        if (is_file(DIR_VHOST_BACKUP.$zipfile)) {
            return URL_VHOST_BACKUP.$zipfile;
        }
    }
    return false;
}
...
function creer_fichier_opml()
{
    ...
    $folders = array();
    foreach ($GLOBALS['feeds_list'] as $i => $feed) {
...
function importer_opml($opmlContent)
{
    ...
    $oldLen = count($GLOBALS['feeds_list']);
    $GLOBALS['feeds_list'] = array_reverse(sort_by_subkey($GLOBALS['feeds_list'], 'title'));
    $GLOBALS['feeds_list'] = array_merge($GLOBALS['array_new'], $GLOBALS['feeds_list']);
    file_put_array(FILE_VHOST_FEEDS_DB, $GLOBALS['feeds_list']);

    return (count($GLOBALS['feeds_list']) - $oldLen);
}
