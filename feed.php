...
function flux_comments_for_article_atom($liste)
{
    $xml = '';
    if (!empty($liste)) {
        $xml .= '<title>'.$GLOBALS['lang']['feed_article_comments_title'].$liste[0]['bt_title'].' - '.$GLOBALS['site_name'].'</title>'."\n";
        $xml .= '<link href="'.$liste[0]['bt_link'].'" />'."\n";
        $xml .= '<id>'.$liste[0]['bt_link'].'</id>';

        foreach ($liste as $comment) {
            $dec = decode_id($comment['bt_id']);
            $tag = 'tag:'.parse_url(URL_ROOT, PHP_URL_HOST).''.$dec['year'].'-'.$dec['month'].'-'.$dec['day'].':'.$comment['bt_id'];
            $xml .= '<entry>'."\n";
                $xml .= '<title>'.$comment['bt_author'].'</title>'."\n";
                $xml .= '<link href="'.$comment['bt_link'].'"/>'."\n";
                $xml .= '<id>'.$tag.'</id>'."\n";
                $xml .= '<updated>'.date('c', mktime($dec['hour'], $dec['minutes'], $dec['seconds'], $dec['month'], $dec['day'], $dec['year'])).'</updated>'."\n";
...
function flux_comments_for_article_rss($liste)
{
    ...
    if (!empty($liste)) {
        ...
        $xml .= '<copyright>'.$GLOBALS['author'].'</copyright>'."\n";
        foreach ($liste as $comment) {
            $dec = decode_id($comment['bt_id']);
            $xml .= '<item>'."\n";
                $xml .= '<title>'.$comment['bt_author'].'</title>'."\n";
                ...
                $xml .= '<pubDate>'.date('r', mktime($dec['hour'], $dec['minutes'], $dec['seconds'], $dec['month'], $dec['day'], $dec['year'])).'</pubDate>'."\n";
...
function rel2abs($article)
{
    ...
    $article = preg_replace('#(src|href)=\"(?!http)#i', '$1="'.URL_ROOT, $article);
    return $article;
}
...
if ($format == 'atom') {
    ...
    $xml .= '<author><name>'.$GLOBALS['author'].'</name></author>'."\n";
...
if (preg_match('#^[0-9]{14}$#', $postId)) {
    ...
    $db_req = '
          ...
          ORDER BY c.bt_id
              DESC';
    $liste = db_items_list($db_req, array((int)$postId), 'commentaires');
 ...
 */
    ...
    if ($format == 'rss') {
        $xml .= '<title>'.$GLOBALS['site_name'].'</title>'."\n";
        $xml .= '<link>'.URL_ROOT.((trim($modes_url, '-') == '') ? '' : '?mode='.(trim($modes_url, '-'))).'</link>'."\n";
        $xml .= '<description><![CDATA['.$GLOBALS['site_description'].']]></description>'."\n";
        $xml .= '<language>fr</language>'."\n";
        $xml .= '<copyright>'.$GLOBALS['author'].'</copyright>'."\n";
...
$xml .= '<!-- generated in '.round(($end - $GLOBALS['BT_timer_start']), 6).' seconds -->'."\n";
if ($format == 'rss') {
    $xml .= '</channel>'."\n";
    $xml .= '</rss>';
} else {
